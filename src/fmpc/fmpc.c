               ...
               double *X, double *U, double *telapsed)
{
    ...
    /* At, Bt */
    F77_CALL(dgemm)
    ("t", "n", &n, &n, &n, &fone, A, &n, eyen, &n, &fzero, At, &n);
    F77_CALL(dgemm)
    ("n", "t", &m, &n, &m, &fone, eyem, &m, B, &n, &fzero, Bt, &m);
    ...
    /* zmaxp, zminp */
    for (i = 0; i < nz; i++)
        zminp[i] = zmin[i] + 0.01 * (zmax[i] - zmin[i]);
    for (i = 0; i < nz; i++)
        zmaxp[i] = zmax[i] - 0.01 * (zmax[i] - zmin[i]);

    /* project z */
    for (i = 0; i < nz; i++)
        z[i] = z[i] > zmaxp[i] ? zmaxp[i] : z[i];
    for (i = 0; i < nz; i++)
        z[i] = z[i] < zminp[i] ? zminp[i] : z[i];

    t1 = clock();
    fmpcsolve(A, B, At, Bt, eyen, eyem, Q, R, Qf, zmax, zmin, x, z,
              T, n, m, nz, niters, kappa, quiet);
    t2 = clock();
...

{
    ...
    for (i = 0; i < m * m; i++)
        Rtilde[i] = R[i];
    F77_CALL(dgemm)
    ("n", "n", &n, &m, &n, &fone, Qf, &n, B, &n, &fzero, tempnm, &n);
    F77_CALL(dgemm)
    ("t", "n", &m, &m, &n, &fone, B, &n, tempnm, &n, &fone, Rtilde, &m);

    /* K */
    F77_CALL(dgemm)
    ("t", "n", &m, &n, &n, &fone, B, &n, Qf, &n, &fzero, tempnm, &m);
    F77_CALL(dgemm)
    ("n", "n", &m, &n, &n, &fone, tempnm, &m, A, &n, &fzero, K, &m);
    for (i = 0; i < m * m; i++)
        Rlower[i] = Rtilde[i];
    F77_CALL(dposv)
    ("l", &m, &n, Rlower, &m, K, &m, &info);
    for (i = 0; i < n * m; i++)
        K[i] = -K[i];

    /* uminp, umaxp, xminp, xmaxp */
    for (i = 0; i < m; i++)
        uminp[i] = umin[i] + 0.01 * (umax[i] - umin[i]);
    for (i = 0; i < m; i++)
        umaxp[i] = umax[i] - 0.01 * (umax[i] - umin[i]);
    for (i = 0; i < n; i++)
        xminp[i] = xmin[i] + 0.01 * (xmax[i] - xmin[i]);
    for (i = 0; i < n; i++)
        xmaxp[i] = xmax[i] - 0.01 * (xmax[i] - xmin[i]);

    t1 = clock();
    for (i = 0; i < nsteps; i++)
    {
        fmpcsolve(A, B, At, Bt, eyen, eyem, Q, R, Qf, zmax, zmin,
                  x, z, T, n, m, nz, niters, kappa, quiet);
        ...
        /* compute x = A*x + B*u + w */
        F77_CALL(dgemv)
        ("n", &n, &n, &fone, A, &n, x, &ione, &fzero, y1, &ione);
        F77_CALL(dgemv)
        ("n", &n, &m, &fone, B, &n, u, &ione, &fzero, y2, &ione);
        F77_CALL(daxpy)
        (&n, &fone, y2, &ione, y1, &ione);
        F77_CALL(daxpy)
        (&n, &fone, w + n * i, &ione, y1, &ione);
        dptr = x;
        dptr1 = y1;
        for (j = 0; j < n; j++)
        {
        ...
        F77_CALL(dgemv)
        ("n", &m, &n, &fone, K, &m, z + nz - 2 * n - m, &ione, &fzero, uterm, &ione);
        F77_CALL(dgemv)
        ("n", &n, &n, &fone, A, &n, z + nz - 2 * n - m, &ione, &fzero, xterm, &ione);
        F77_CALL(dgemv)
        ("n", &n, &m, &fone, B, &n, u, &ione, &fzero, y2, &ione);
        F77_CALL(daxpy)
        (&n, &fone, y2, &ione, xterm, &ione);
        for (j = 0; j < m; j++)
            uterm[j] = uterm[j] > umaxp[j] ? umaxp[j] : uterm[j];
        for (j = 0; j < m; j++)
            uterm[j] = uterm[j] < uminp[j] ? uminp[j] : uterm[j];
        for (j = 0; j < n; j++)
            xterm[j] = xterm[j] > xmaxp[j] ? xmaxp[j] : xterm[j];
        for (j = 0; j < n; j++)
            xterm[j] = xterm[j] < xminp[j] ? xminp[j] : xterm[j];
               ...
               int quiet)
{
    ...
    F77_CALL(dgemv)
    ("n", &n, &n, &fone, A, &n, x, &ione, &fzero, b, &ione);
    dptr = z;
    dptr1 = z0;
    for (i = 0; i < nz; i++)
    {
    ...
    for (iiter = 0; iiter < maxiter; iiter++)
    {
        ...
        while (1)
        {
            ...
            for (i = 0; i < nz; i++)
            {
                if (*dptr + s * (*dptr1) >= *dptr2)
                    cont = 1;
                if (*dptr + s * (*dptr1) <= *dptr3)
                    cont = 1;
                dptr++;
                dptr1++;
                dptr2++;
           ...
           double *rp, int T, int n, int m, int nz, double kappa, double *dnu, double *dz)
{
    ...
    for (i = 0; i < T; i++)
    {
        ...
        dptr1 = PhiQ + n * n * i;
        F77_CALL(dposv)
        ("l", &n, &n, dptr1, &n, dptr, &n, &info);
        dptr = PhiinvQeye + n * n * i;
        dptr1 = eyen;
        for (j = 0; j < n * n; j++)
        {
            *dptr = *dptr1;
        ...
        dptr1 = PhiQ + n * n * i;
        F77_CALL(dtrtrs)
        ("l", "n", "n", &n, &n, dptr1, &n, dptr, &n, &info);
        F77_CALL(dtrtrs)
        ("l", "t", "n", &n, &n, dptr1, &n, dptr, &n, &info);
    }
    for (i = 0; i < T; i++)
    {
        dptr = PhiinvRBt + m * n * i;
        dptr1 = Bt;
        for (j = 0; j < n * m; j++)
        {
            *dptr = *dptr1;
        ...
        dptr1 = PhiR + m * m * i;
        F77_CALL(dposv)
        ("l", &m, &n, dptr1, &m, dptr, &m, &info);
        dptr = PhiinvReye + m * m * i;
        dptr1 = eyem;
        for (j = 0; j < m * m; j++)
        {
            *dptr = *dptr1;
        ...
        dptr1 = PhiR + m * m * i;
        F77_CALL(dtrtrs)
        ("l", "n", "n", &m, &m, dptr1, &m, dptr, &m, &info);
        F77_CALL(dtrtrs)
        ("l", "t", "n", &m, &m, dptr1, &m, dptr, &m, &info);
    }
    ...
    dptr2 = dptr2 - n * n;
    F77_CALL(dgemm)
    ("n", "n", &n, &n, &m, &fone, B, &n, PhiinvRBt, &m, &fone, dptr2, &n);
    for (i = 1; i < T; i++)
    {
        dptr = Yd + n * n * i;
        dptr1 = PhiinvQeye + n * n * i;
        for (j = 0; j < n * n; j++)
        {
            *dptr = *dptr1;
            dptr++;
        ...
        dptr1 = PhiinvRBt + m * n * i;
        dptr = dptr - n * n;
        F77_CALL(dgemm)
        ("n", "n", &n, &n, &m, &fone, B, &n, dptr1, &m, &fone, dptr, &n);
        dptr1 = PhiinvQAt + n * n * (i - 1);
        F77_CALL(dgemm)
        ("n", "n", &n, &n, &n, &fone, A, &n, dptr1, &n, &fone, dptr, &n);
    }
    ...
    for (i = 1; i < T; i++)
    {
        ...
        dptr2 = dptr2 - n * n;
        F77_CALL(dgemm)
        ("t", "n", &n, &n, &n, &fone, dptr1, &n, eyen, &n, &fzero, dptr2, &n);
        dptr = Ld + n * n * i;
        dptr1 = Yd + n * n * i;
        for (j = 0; j < n * n; j++)
        {
        ...
        dptr = dptr - n * n;
        F77_CALL(dgemm)
        ("n", "t", &n, &n, &n, &fmone, dptr2, &n, dptr2, &n, &fone, dptr, &n);
        F77_CALL(dposv)
        ("l", &n, &ione, dptr, &n, temp, &n, &info);
    }
    ...
    dptr = dptr - n;
    F77_CALL(dtrsv)
    ("l", "n", "n", &n, PhiQ, &n, dptr, &ione);
    F77_CALL(dtrsv)
    ("l", "t", "n", &n, PhiQ, &n, dptr, &ione);
    dptr2 = temp;
    dptr1 = rd;
    for (i = 0; i < m; i++)
    {
    ...
    dptr2 = dptr2 - m;
    F77_CALL(dtrsv)
    ("l", "n", "n", &m, PhiR, &m, dptr2, &ione);
    F77_CALL(dtrsv)
    ("l", "t", "n", &m, PhiR, &m, dptr2, &ione);
    F77_CALL(dgemv)
    ("n", &n, &m, &fmone, B, &n, temp, &ione, &fone, dptr, &ione);
    ...
    for (i = 1; i < T; i++)
    {
        ...
        dptr3 = PhiQ + n * n * i;
        F77_CALL(dtrsv)
        ("l", "n", "n", &n, dptr3, &n, dptr, &ione);
        F77_CALL(dtrsv)
        ("l", "t", "n", &n, dptr3, &n, dptr, &ione);
        dptr2 = temp;
        dptr1 = rd + i * (m + n);
        for (j = 0; j < m; j++)
        {
        ...
        dptr2 = dptr2 - m;
        F77_CALL(dtrsv)
        ("l", "n", "n", &m, dptr3, &m, dptr2, &ione);
        F77_CALL(dtrsv)
        ("l", "t", "n", &m, dptr3, &m, dptr2, &ione);
        F77_CALL(dgemv)
        ("n", &n, &m, &fmone, B, &n, temp, &ione, &fone, dptr, &ione);
        dptr2 = temp;
        dptr1 = rd + (i - 1) * (n + m) + m;
        ...
        dptr2 = dptr2 - n;
        F77_CALL(dtrsv)
        ("l", "n", "n", &n, dptr3, &n, dptr2, &ione);
        F77_CALL(dtrsv)
        ("l", "t", "n", &n, dptr3, &n, dptr2, &ione);
        F77_CALL(dgemv)
        ("n", &n, &n, &fmone, A, &n, temp, &ione, &fone, dptr, &ione);
    }
    ...
    for (i = 1; i < T; i++)
    {
        ...
        dptr3 = Lld + n * n * (i - 1);
        F77_CALL(dgemv)
        ("n", &n, &n, &fmone, dptr3, &n, dptr1, &ione, &fmone, dptr, &ione);
        dptr3 = Ld + n * n * i;
        F77_CALL(dtrsv)
        ("l", "n", "n", &n, dptr3, &n, dptr, &ione);
    }
    ...
    for (i = T - 1; i > 0; i--)
    {
        ...
        dptr3 = Lld + n * n * (i - 1);
        F77_CALL(dgemv)
        ("t", &n, &n, &fmone, dptr3, &n, dptr1, &ione, &fone, dptr, &ione);
        dptr3 = Ld + n * n * (i - 1);
        F77_CALL(dtrsv)
        ("l", "t", "n", &n, dptr3, &n, dptr, &ione);
    }
    ...
    for (i = 0; i < T - 1; i++)
    {
        ...
        dptr1 = dnu + i * n;
        F77_CALL(dgemv)
        ("n", &m, &n, &fmone, Bt, &m, dptr1, &ione, &fzero, dptr, &ione);
        dptr = Ctdnu + i * (n + m) + m;
        dptr2 = dnu + (i + 1) * n;
        for (j = 0; j < n; j++)
        {
        ...
        dptr = dptr - n;
        F77_CALL(dgemv)
        ("n", &n, &n, &fmone, At, &n, dptr2, &ione, &fone, dptr, &ione);
    }
    ...
    {
        dptr = dz + (i + 1) * m + i * n;
        dptr1 = rdmCtdnu + (i + 1) * m + i * n;
        dptr2 = PhiinvQeye + n * n * i;
        F77_CALL(dgemv)
        ("n", &n, &n, &fone, dptr2, &n, dptr1, &ione, &fzero, dptr, &ione);
    }
    for (i = 0; i < T; i++)
    {
        dptr = dz + i * (m + n);
        dptr1 = rdmCtdnu + i * (m + n);
        dptr2 = PhiinvReye + m * m * i;
        F77_CALL(dgemv)
        ("n", &m, &m, &fone, dptr2, &m, dptr1, &ione, &fzero, dptr, &ione);
    }
    free(PhiQ);
    free(PhiR);
    free(PhiinvQAt);
          ...
          double kappa, double *rd, double *rp, double *Ctnu)
{
    ...
    F77_CALL(dgemv)
    ("n", &n, &m, &fmone, B, &n, z, &ione, &fone, Cz, &ione);
    for (i = 2; i <= T; i++)
    {
        dptr = Cz + (i - 1) * n;
        dptr1 = z + m + (i - 2) * (n + m);
        ...
        dptr = dptr - n;
        F77_CALL(dgemv)
        ("n", &n, &n, &fmone, A, &n, dptr1, &ione, &fone, dptr, &ione);
        dptr1 = dptr1 + n;
        F77_CALL(dgemv)
        ("n", &n, &m, &fmone, B, &n, dptr1, &ione, &fone, dptr, &ione);
    }
    /*
    ...
    */
    ...
    {
        F77_CALL(dgemv)
        ("t", &n, &m, &fmone, B, &n, dptr2, &ione, &fzero, dptr, &ione);
        dptr = dptr + n + m;
        for (j = 0; j < n; j++)
        {
            *dptr1 = *dptr2;
        ...
        F77_CALL(dgemv)
        ("t", &n, &n, &fmone, A, &n, dptr2, &ione, &fone, dptr1, &ione);
        dptr1 = dptr1 + n + m;
    }
    F77_CALL(dgemv)
    ("t", &n, &m, &fmone, B, &n, dptr2, &ione, &fzero, dptr, &ione);
            ...
            int T, int n, int m, int nz, double *gf, double *gp, double *hp)
{
    ...
    {
        F77_CALL(dgemv)
        ("n", &m, &m, &ftwo, R, &m, dptr1, &ione, &fzero, dptr, &ione);
        dptr = dptr + m;
        dptr1 = dptr1 + m;
        F77_CALL(dgemv)
        ("n", &n, &n, &ftwo, Q, &n, dptr1, &ione, &fzero, dptr, &ione);
        dptr = dptr + n;
        dptr1 = dptr1 + n;
    }
    F77_CALL(dgemv)
    ...
    dptr1 = dptr1 + m;
    F77_CALL(dgemv)
    ("n", &n, &n, &ftwo, Qf, &n, dptr1, &ione, &fzero, dptr, &ione);
